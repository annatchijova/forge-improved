...
def test_coverage_accounting_never_loses_readable_non_python_files(tmp_path):
    ...
    # Prose is not an audit gap. It belongs in its own bucket, apart from
    # source FORGE could not analyse and source it chose not to.
    assert "README.md" in coverage["skipped_reasons"].get("non_source_not_analyzed", ())
    assert coverage["language_coverage"]["Python"] == {"analyzed": 1, "abstained": 0, "depth": "ast"}
    assert coverage["language_coverage"]["MD"] == {"analyzed": 0, "abstained": 1, "depth": "none"}


def test_coverage_separates_scope_from_engine_limit_and_records_depth(tmp_path):
    put(tmp_path, "main.py", "x = 1\n")
